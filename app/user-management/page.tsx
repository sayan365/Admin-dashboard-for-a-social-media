...
import React, { useEffect, useState } from 'react';
import { fetchUserActivity } from '../utils/api'; // Adjusted to your API setup
...
const UserManagement = () => {
  ...
  useEffect(() => {
    const getMetrics = async () => {
      ...
      if (data && data.dashboard && data.dashboard.userMetrics) {
        setMetrics(data.dashboard.userMetrics.daily); // Set metrics with the correct type
      }
      setLoading(false);
    };
    getMetrics();
  ...
  // Pie Chart Data
  const pieChartData = [
    { name: 'Active Users', value: metrics?.activeUser || 0 }, // Use optional chaining and fallback to 0 if null
    { name: 'Creators', value: metrics?.creator || 0 }, // Same for other properties
  ];
